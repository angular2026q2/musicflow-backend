...
## Entry #9 
**createdAt:** 12 May 2026, 00:11

### Day-Plan Thoughts
Three major modules complete today: `Music`, `Favorites`, and `Listening History`.  

`Music` module require studying the `Jamendo API` response structure before writing any code.  

`Favorites` and `Listening History` are similar in structure but differ in one key detail (mind!) - `favorites` enforce _uniqueness_ per user/track, while `listening_history` 
allows duplicates (same track can be played multiple times).

### Implementation

**Music module** (`src/music/`):
- Added `JamendoTrack`, `JamendoAlbum`, `JamendoArtist`, `JamendoResponse` interfaces based on real Jamendo API responses; 
- Added `PaginatedResult<T>` wrapper with `data` and `meta` fields (`results_count`, `next`)
- Added `MusicQueryDto` with `search`, `tags`, `limit`, `offset` params
- Added `MusicService` with `getTracks`, `getTrackById`, `getAlbums`, `getAlbumById`, `getArtists`, `getArtistById`, `getFeaturedTracks`, `getGenres` methods
- Added `MusicController` with all endpoints protected by `JwtAuthGuard`
- `tags` param is `string` (not `string[]`) because Jamendo accepts comma-separated values: `tags=rock,jazz`
- `client_id` is safe to use server-side — it's a public Jamendo identifier, never exposed to the client

**Favorites module** (`src/favorites/`):
- created `favorites` table with separate typed columns, created policies for all methods in SQL Editor
- Added `unique(user_id, track_id)` constraint at DB level to prevent duplicate in `favorites`
- Add:
  - `AddFavoriteDto`, 
  - `FavoritesService`, 
  - `FavoritesController`, 
...
- created endpoints: 
  ...
  - `GET /favorites/:trackId/check`
- Error code `23505` from Postgres = unique constraint violation → returns `409 Conflict`

**Listening History module** (`src/history/`):
- Recreated `listening_history` table with same separate field structure as favorites
- No `unique` constraint, same track can appear multiple times in history
...
- `getHistory` supports optional `limit` param (default 50)

**Database types regenerated** after both table recreations via:

```shell
./node_modules/.bin/supabase gen types typescript --project-id ... > src/types/database.types.ts
```

placed `/* eslint-disable */` at the top of the file to suppress warnings on autogenerated code.

### Problems & Solutions

**Problem:** `npx supabase gen types` wrote the npm install prompt into the file instead of the types.
**Solution:** Installed `supabase` locally as dev dependency and used `./node_modules/.bin/supabase` instead.

**Problem:** `listening_history` table already existed with old `track_data jsonb` structure.
**Solution:** Dropped the old table and recreated with separate typed fields, then regenerated types.

**Problem:** Missing semicolon after `CREATE TABLE` closing parenthesis caused SQL syntax error.
**Solution:** Added `;` after `)` before the `ALTER TABLE` statement.
